...
class MicroseismicDataset(Dataset):
    def __init__(self, data_path, sequence_length=5, prediction_days=3, spatial_size=(60, 15, 5), augment=False):
        ...
        # 设置日期列表
        self.dates = self.data['datetime'].dt.date.unique()
    ...
    def _create_daily_grid(self, date):
        ...
        if len(day_data) == 0:
            return grid
        print(f"Processing date: {date}, Events count: {len(day_data)}")
        print(f"Energy range: [{day_data['energy'].min():.4f}, {day_data['energy'].max():.4f}]")    
        # 计算实际坐标到网格索引的映射
        ...
        # 数据增强
        if self.augment:
            grid = self._augment_data(grid)
        
        return grid
    
    def _augment_data(self, grid):
        """数据增强函数"""
        if random.random() > 0.5:
            grid = np.flip(grid, axis=1)  # 水平翻转
        if random.random() > 0.5:
            grid = np.flip(grid, axis=2)  # 垂直翻转
        if random.random() > 0.5:
            grid = np.flip(grid, axis=3)  # 深度翻转
        return grid
...
def split_and_save_data(data_path, output_dir, train_ratio=0.7, val_ratio=0.2):
    ...
    if train_days + val_days >= total_days:
        raise ValueError("Train and validation ratio sum must be less than 1.")
